...
| D10 | File logger | Retain and relocate implementation to Infrastructure | Superseded after approved migration gates | Retain NDJSON behavior and neutral contracts for the ResultPattern migration. The later replacement is now approved by [`error-observability-policy.md`](error-observability-policy.md); NDJSON remains until that policy's logging, exception-boundary, and deletion gates are satisfied. |
