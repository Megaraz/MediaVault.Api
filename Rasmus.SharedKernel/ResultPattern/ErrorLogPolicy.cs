...
{
    public bool ShouldLog(Megaraz.ResultPattern.Error error)
    {
        return error switch
        {
            Megaraz.ResultPattern.ValidationError => false,
            DatabaseError => true,
            HttpError httpError => ShouldLogHttpError(httpError),
            _ when error.Type == Megaraz.ResultPattern.ErrorType.Cancelled => false,
            _ => true
        };
    }
